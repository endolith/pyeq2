...
class ExtendedVersionHandler_ExponentialGrowth(IExtendedVersionHandler.IExtendedVersionHandler):
    ...
    def AssembleDisplayHTML(self, inModel):
        ...
        if inModel.baseEquationHasGlobalMultiplierOrDivisor_UsedInExtendedVersions:
            return f'{inModel._HTML}<br>{inModel._leftSideHTML} = {inModel._leftSideHTML} * exp({x_or_xy})'

        try:
            cd = inModel.GetCoefficientDesignators()
            return f'{inModel._HTML}<br>{inModel._leftSideHTML} = {inModel._leftSideHTML} * ({cd[-1]} * exp({x_or_xy}))'
    ...
    def AssembleDisplayName(self, inModel):
        return f'{inModel._baseName} With Exponential Growth'
        ...
        if inModel.GetDimensionality() == 2:
            return (
                ...
                if inModel.baseEquationHasGlobalMultiplierOrDivisor_UsedInExtendedVersions
                    ...
                    * (inCoeffs[len(inCoeffs) - 1] * inDataCacheDictionary['ExpX'])
                )
            )
