...
def SaveCoefficientAndFitStatistics(in_filePathFitStatistics, in_equation):
    ...
    with open(in_filePathFitStatistics, 'w') as outputFile:
        ...
        outputFile.write(f'Degress of freedom error {str(in_equation.df_e)}' + '\n')
        outputFile.write(
            f'Degress of freedom regression {str(in_equation.df_r)}' + '\n'
        )


        if in_equation.rmse is None:
            outputFile.write('Root Mean Squared Error (RMSE): n/a\n')
        else:
            outputFile.write(
                f'Root Mean Squared Error (RMSE): {str(in_equation.rmse)}'
                + '\n'
            )


        if in_equation.r2 is None:
            outputFile.write('R-squared: n/a\n')
        else:
            outputFile.write(f'R-squared: {str(in_equation.r2)}' + '\n')

        if in_equation.r2adj is None:
            outputFile.write('R-squared adjusted: n/a\n')
        else:
            outputFile.write(f'R-squared adjusted: {str(in_equation.r2adj)}' + '\n')

        if in_equation.Fstat is None:
            outputFile.write('Model F-statistic: n/a\n')
        else:
            outputFile.write(f'Model F-statistic: {str(in_equation.Fstat)}' + '\n')

        if in_equation.Fpv is None:
            outputFile.write('Model F-statistic p-value: n/a\n')
        else:
            outputFile.write(f'Model F-statistic p-value: {str(in_equation.Fpv)}' + '\n')

        if in_equation.ll is None:
            outputFile.write('Model log-likelihood: n/a\n')
        else:
            outputFile.write(f'Model log-likelihood: {str(in_equation.ll)}' + '\n')

        if in_equation.aic is None:
            outputFile.write('Model AIC: n/a\n')
        else:
            outputFile.write(f'Model AIC: {str(in_equation.aic)}' + '\n')

        if in_equation.bic is None:
            outputFile.write('Model BIC: n/a\n')
        else:
            outputFile.write(f'Model BIC: {str(in_equation.bic)}' + '\n')

        outputFile.write('\n')
        outputFile.write("Individual Parameter Statistics:\n")
        for i in range(len(in_equation.solvedCoefficients)):
            if in_equation.tstat_beta is None:
                tstat = 'n/a'
            else:
                tstat = '%-.5E' %  ( in_equation.tstat_beta[i])

            if in_equation.pstat_beta is None:
                pstat = 'n/a'
            else:
                pstat = '%-.5E' %  ( in_equation.pstat_beta[i])

            if in_equation.sd_beta is None:
...
def SaveSourceCode(in_sourceCodeFilePath,  in_equation):
    ...
    with open(in_sourceCodeFilePath, 'w') as outputFile:
